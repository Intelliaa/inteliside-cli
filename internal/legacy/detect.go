...
const sddLegacyMarker = "<!-- sdd-legacy:generated -->"
...
func Detect(projectDir string) *Artifacts {
	...
	// If it was generated by SDD-Legacy, DON'T archive — it has real analysis data.
	// The CLI should MERGE (add ATL section) instead of replacing with generic templates.
	if strings.Contains(content, sddLegacyMarker) {
		a.IsLegacy = false // not "legacy" in the archive sense
		a.ClaudeMD = claudeMD
		a.ClaudeMDBody = content
		a.ExtractedVars = ExtractVars(content)
		// Also check .claude/CLAUDE.md — preserve if SDD-Legacy generated
		...
		fmt.Println("  ℹ CLAUDE.md generado por SDD-Legacy detectado — se preservará y se agregará config ATL")
		return a
	}

	// It has a CLAUDE.md without any markers → truly legacy, archive it
	a.IsLegacy = true
	a.ClaudeMD = claudeMD
	a.ClaudeMDBody = content
